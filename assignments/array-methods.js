...
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the 
// director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. 
// This array will contain just strings.
let firstNamesAllCaps = [];
runners.map( (capsNames) => firstNamesAllCaps.push(`${capsNames.first_name.toUpperCase()}`))
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. 
//We need a filtered version of the runners array, containing only those runners with large 
//sized shirts so they can choose a different size. This will be an array of objects.
...
let ticketPriceTotal = runners.reduce((accumulator, item) => accumulator + item.donation, 0);
...
//set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one 
//or many of the array methods listed above.

// Problem 1: Who donated over $200?
let largeDonation = [];
